...
//                               &type_str);
  type_str = jmessage.get(kSessionDescriptionTypeName, std::string()).asString();
...
//                                          &sdp)) {
    if (!jmessage.isMember(kSessionDescriptionSdpName)) {
      RTC_LOG(WARNING) << "Can't parse received session description message.";
      return;
    }
    sdp = jmessage[kSessionDescriptionSdpName].asString();
...
//            !rtc::GetStringFromJsonObject(jmessage, kCandidateSdpName, &sdp)) {
    if (!jmessage.isMember(kCandidateSdpMidName) ||
        !jmessage.isMember(kCandidateSdpMlineIndexName) ||
        !jmessage.isMember(kCandidateSdpName)) {
      RTC_LOG(WARNING) << "Can't parse received message.";
      return;
    }
    sdp_mid = jmessage[kCandidateSdpMidName].asString();
    sdp_mlineindex = jmessage[kCandidateSdpMlineIndexName].asInt();
    sdp = jmessage[kCandidateSdpName].asString();
    webrtc::SdpParseError error;
